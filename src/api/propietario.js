...
async function getAll(establesimientoId) {
  try {
    ...
    const url = `${ENV.API_URL}/${ENV.ENDPOINTS.PROPIETARIO}?${filters}`;
...
async function createData(data) {
  try {
    const url = `${ENV.API_URL}/${ENV.ENDPOINTS.PROPIETARIO}`;
    const params = {
      method: "POST",
      headers: {
...
async function getById(identificador) {
  try {
    const url = `${ENV.API_URL}/${ENV.ENDPOINTS.PROPIETARIO}/${identificador}`;
    const response = await authFetch(url);
    if (response.status !== 200) throw response;
