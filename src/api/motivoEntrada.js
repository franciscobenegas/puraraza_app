...
async function getAll(establesimientoId) {
  try {
    ...
    const url = `${ENV.API_URL}/${ENV.ENDPOINTS.MOTIVO_ENTRADA}?${filters}`;
...
async function createData(data) {
  try {
    const url = `${ENV.API_URL}/${ENV.ENDPOINTS.MOTIVO_ENTRADA}`;
    const params = {
      method: "POST",
      headers: {
...
async function getById(identificador) {
  try {
    const url = `${ENV.API_URL}/${ENV.ENDPOINTS.MOTIVO_ENTRADA}/${identificador}`;
    const response = await authFetch(url);
    if (response.status !== 200) throw response;
...
export const motivoEntradaCtrl = {
  getAll: getAll,
  getId: getById,
  create: createData,
  update: updateData,
